...
namespace Bingo
{
    class Program
    {
        ...
        private static void CommandLoop()
        {
            string command = "";
            string[] commandWords;
            Console.Write("Welcome to Dawson's Dutch Bingo Parlor!\n");
            ...
            while (command != "exit")
            {
                ...
                // dump command prints out the graph
                    ...
                    rg.Descendants(commandWords[1], 0, out failed);

                else if(command == "bingo")
                    rg.Bingo(commandWords[1], commandWords[2]);

                else if(command == "cousins")
                {
                    if(int.TryParse(commandWords[2], out _) && int.TryParse(commandWords[3], out _))
                    {
